...
var dbg = function(s) {
	//if(typeof console !== 'undefined') console.log("Readability: " + s);
};
...
var readability = {
	 ...
	 * 
	 * Workflow:
	 *  1. Prep the docFrame by removing script tags, css, etc.
	...
	init: function(preserveUnlikelyCandidates) {
		...
		if(docFrame.body && !readability.bodyCache)
			readability.bodyCache = docFrame.body.innerHTML;
		
		readability.prepDocument();
		
		/* Build readability's DOM tree */
		var overlay        = docFrame.createElement("DIV");
		var innerDiv       = docFrame.createElement("DIV");
		...
		**/
		if(readability.getInnerText(articleContent, false) == "")
		{
			if(!preserveUnlikelyCandidates) {
				docFrame.body.innerHTML = readability.bodyCache;
		...
		docFrame.body.className = readStyle;
		overlay.className       = readStyle;
		innerDiv.className      = readMargin + " " + readSize;

		/* Glue the structure of our docFrame together. */
		...
		docFrame.body.innerHTML = "";
		docFrame.body.insertBefore(overlay, docFrame.body.firstChild);

		if(readability.frameHack)
		{
			var readOverlay = docFrame.getElementById('readOverlay');
	...
	/**
	 * Get the article title as an H1. Currently just uses docFrame.title,
	 * we might want to be smarter in the future.
	 *
	 * @return void
	 **/
	getArticleTitle: function () {
		var articleTitle = docFrame.createElement("H1");
		articleTitle.innerHTML = docFrame.title;
	...
	/**
	 * Prepare the HTML docFrame for readability to scrape it.
	...
	prepDocument: function () {
		 ...
		 * so we create a new body node and append it to the docFrame.
		 */
		if(docFrame.body === null)
		{
			body = docFrame.createElement("body");
			try {
				docFrame.body = body;		
			}
			catch(e) {
				docFrame.docFrameElement.appendChild(body);
			}
		}

		var frames = docFrame.getElementsByTagName('frame');
		if(frames.length > 0)
		{
			var bestFrame = null;
			var bestFrameSize = 0;
			for(var frameIndex = 0; frameIndex < frames.length; frameIndex++)
			{
				...
				try {
					frames[frameIndex].contentWindow.docFrame.body;
					canAccessFrame = true;
				} catch(e) {}
			...
			if(bestFrame)
			{
				var newBody = docFrame.createElement('body');
				newBody.innerHTML = bestFrame.contentWindow.docFrame.body.innerHTML;
				newBody.style.overflow = 'scroll';
				docFrame.body = newBody;
				
				var frameset = docFrame.getElementsByTagName('frameset')[0];
				if(frameset)
		...
		if (readStyle == "style-classy") {
			var typeKitScript  = docFrame.createElement('script');
			typeKitScript.type = "text/javascript";
			typeKitScript.src  = "http://use.typekit.com/sxt6vzy.js";

			docFrame.body.appendChild(typeKitScript);

			/**
			 * Done as a script elem so that it's ensured it will activate
			 * after typekit is loaded from the previous script src.
			**/
			var typeKitLoader  = docFrame.createElement('script');
			typeKitLoader.type = "text/javascript";

			var typeKitLoaderContent = docFrame.createTextNode('try{Typekit.load();}catch(e){}');
			typeKitLoader.appendChild(typeKitLoaderContent);
			docFrame.body.appendChild(typeKitLoader);
		}

		/* remove all scripts that are not readability */
		var scripts = docFrame.getElementsByTagName('script');
		...
		var styleTags = docFrame.getElementsByTagName("style");
		...
		docFrame.body.innerHTML = docFrame.body.innerHTML.replace(readability.regexps.replaceBrsRe, '</p><p>').replace(readability.regexps.replaceFontsRe, '<$1span>')
	},
	...
	**/
	grabArticle: function (preserveUnlikelyCandidates) {
		...
		**/
		for(var nodeIndex = 0; (node = docFrame.getElementsByTagName('*')[nodeIndex]); nodeIndex++)
		{
			...
			if (node.tagName === "DIV") {
				if (node.innerHTML.search(readability.regexps.divToPElementsRe) === -1)	{
					...
					var newNode = docFrame.createElement('p');
					try {
				...
				else
				{
					...
					for(var i = 0, il = node.childNodes.length; i < il; i++) {
						...
						if(childNode.nodeType == Node.TEXT_NODE) {
							...
							var p = docFrame.createElement('p');
		...
		**/
		var allParagraphs = docFrame.getElementsByTagName("p");
		var candidates    = [];
		...
		**/
		...
		{
			topCandidate = docFrame.createElement("DIV");
			topCandidate.innerHTML = docFrame.body.innerHTML;
			docFrame.body.innerHTML = "";
			docFrame.body.appendChild(topCandidate);
		...
		**/
		var articleContent        = docFrame.createElement("DIV");
	...
	**/
	 ...
	 **/
	emailBox: function () {
	    var emailContainer = docFrame.getElementById('email-container');
	    if(null != emailContainer)
	    {
	        return;
	    }

	    var emailContainer = docFrame.createElement('div');
	    emailContainer.setAttribute('id', 'email-container');
	    emailContainer.innerHTML = '<iframe src="'+readability.emailSrc + '?pageUrl='+escape(window.location)+'&pageTitle='+escape(docFrame.title)+'" scrolling="no" onload="readability.removeFrame()" style="width:500px; height: 490px; border: 0;"></iframe>';

	    docFrame.body.appendChild(emailContainer);			
	},
	 ...
	 **/
	kindleBox: function () {
	    var kindleContainer = docFrame.getElementById('kindle-container');
	    if(null != kindleContainer)
	    {
	        return;
	    }

	    var kindleContainer = docFrame.createElement('div');
	    ...
	    docFrame.body.appendChild(kindleContainer);
		...
		var formHtml =  '<form id="readabilityKindleForm" style="display: none;" target="readabilityKindleIframe" method="post" action="' + readability.kindleSrc + '">\
		                     <input type="hidden" name="bodyContent" id="bodyContent" value="' + readability.htmlspecialchars(docFrame.getElementById('readability-content').innerHTML) + '" />\
						     <input type="hidden" name="pageUrl" id="pageUrl" value="' + readability.htmlspecialchars(window.location) + '" />\
						     <input type="hidden" name="pageTitle" id="pageUrl" value="' + readability.htmlspecialchars(docFrame.title) + '" />\
                         </form>';

		docFrame.body.innerHTML += formHtml;
		docFrame.forms['readabilityKindleForm'].submit();
	},
